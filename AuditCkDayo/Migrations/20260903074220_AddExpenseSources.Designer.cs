// <auto-generated />
using System;
using AuditCkDayo.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace AuditCkDayo.Migrations
{
    [DbContext(typeof(AuditDbContext))]
    [Migration("20260903074220_AddExpenseSources")]
    partial class AddExpenseSources
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("AuditCkDayo.Models.AuditItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int?>("AssignedReviewerId")
                        .HasColumnType("int");

                    b.Property<int>("BuyerId")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime>("EntryDate")
                        .HasColumnType("date");

                    b.Property<int>("EstablishmentId")
                        .HasColumnType("int");

                    b.Property<string>("Notes")
                        .HasColumnType("longtext");

                    b.Property<string>("ReceiptImageUrl")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasDefaultValue("AwaitingBranchVerification");

                    b.Property<DateTime?>("SubmittedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("VerificationDate")
                        .HasColumnType("datetime(6)");

                    b.Property<int?>("VerifiedById")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AssignedReviewerId");

                    b.HasIndex("BuyerId");

                    b.HasIndex("EstablishmentId");

                    b.HasIndex("VerifiedById");

                    b.ToTable("AuditItems");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditItemDetail", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AllocationNotes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<int?>("AssignedEstablishmentId")
                        .HasColumnType("int");

                    b.Property<int>("AuditItemId")
                        .HasColumnType("int");

                    b.Property<string>("BranchVerificationStatus")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasDefaultValue("Pending");

                    b.Property<int?>("CostCenterId")
                        .HasColumnType("int");

                    b.Property<int?>("ExpenseSourceId")
                        .HasColumnType("int");

                    b.Property<string>("ExpenseSourceName")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("ItemName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)");

                    b.Property<int?>("PnlCategoryId")
                        .HasColumnType("int");

                    b.Property<string>("PnlCategoryName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("PnlSection")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<decimal>("Price")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int>("Quantity")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.Property<string>("ReceiptStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<decimal>("Total")
                        .HasColumnType("decimal(12,2)");

                    b.HasKey("Id");

                    b.HasIndex("AssignedEstablishmentId");

                    b.HasIndex("AuditItemId");

                    b.HasIndex("CostCenterId");

                    b.HasIndex("ExpenseSourceId");

                    b.HasIndex("PnlCategoryId");

                    b.ToTable("AuditItemDetails");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditItemImage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AuditItemId")
                        .HasColumnType("int");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("int");

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.HasKey("Id");

                    b.HasIndex("AuditItemId");

                    b.ToTable("AuditItemImages");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditSettlement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("ActualChangeReturned")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("ExpectedChange")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int?>("PcfReleaseId")
                        .HasColumnType("int");

                    b.Property<int>("ProcessedByUserId")
                        .HasColumnType("int");

                    b.Property<string>("ReceiverName")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<int?>("ReceiverUserId")
                        .HasColumnType("int");

                    b.Property<int>("ResponsibleManagerId")
                        .HasColumnType("int");

                    b.Property<decimal>("ShortOverAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<decimal>("TotalAcceptedExpenses")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("TotalPCReleased")
                        .HasColumnType("decimal(12,2)");

                    b.HasKey("Id");

                    b.HasIndex("PcfReleaseId");

                    b.HasIndex("ProcessedByUserId");

                    b.HasIndex("ReceiverUserId");

                    b.HasIndex("ResponsibleManagerId");

                    b.ToTable("AuditSettlements");
                });

            modelBuilder.Entity("AuditCkDayo.Models.CashBreakdownLine", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Denomination")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int>("OwnerId")
                        .HasColumnType("int");

                    b.Property<string>("OwnerType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<int?>("SalesReportId")
                        .HasColumnType("int");

                    b.Property<int>("Section")
                        .HasColumnType("int");

                    b.Property<decimal>("Total")
                        .HasColumnType("decimal(12,2)");

                    b.HasKey("Id");

                    b.HasIndex("SalesReportId");

                    b.ToTable("CashBreakdownLines");
                });

            modelBuilder.Entity("AuditCkDayo.Models.CashFlowEntry", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<int?>("ConfirmedByUserId")
                        .HasColumnType("int");

                    b.Property<int?>("CostCenterId")
                        .HasColumnType("int");

                    b.Property<int>("CreatedByUserId")
                        .HasColumnType("int");

                    b.Property<string>("Direction")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)");

                    b.Property<int?>("EstablishmentId")
                        .HasColumnType("int");

                    b.Property<string>("Notes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<int?>("RelatedUserId")
                        .HasColumnType("int");

                    b.Property<int?>("ReportedByUserId")
                        .HasColumnType("int");

                    b.Property<int?>("SourceDocumentId")
                        .HasColumnType("int");

                    b.Property<int>("TreasuryCashFlowId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ConfirmedByUserId");

                    b.HasIndex("CostCenterId");

                    b.HasIndex("CreatedByUserId");

                    b.HasIndex("EstablishmentId");

                    b.HasIndex("RelatedUserId");

                    b.HasIndex("ReportedByUserId");

                    b.HasIndex("TreasuryCashFlowId");

                    b.HasIndex("SourceDocumentId", "Category")
                        .IsUnique();

                    b.ToTable("CashFlowEntries");
                });

            modelBuilder.Entity("AuditCkDayo.Models.CostCenter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("Notes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("CostCenters");
                });

            modelBuilder.Entity("AuditCkDayo.Models.DocumentRecord", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("ConfirmedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<int?>("ConfirmedByUserId")
                        .HasColumnType("int");

                    b.Property<string>("DocumentType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("OcrRawJson")
                        .HasColumnType("longtext");

                    b.Property<string>("OcrStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("ReviewStatus")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("UploadedByUserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ConfirmedByUserId");

                    b.HasIndex("UploadedByUserId");

                    b.ToTable("DocumentRecords");
                });

            modelBuilder.Entity("AuditCkDayo.Models.Establishment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("DailyStartingFloat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(12,2)")
                        .HasDefaultValue(0.00m);

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(true);

                    b.Property<bool>("IsMiscellaneous")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsOperatingBranch")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<decimal>("PcfBalance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(12,2)")
                        .HasDefaultValue(0.00m);

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Establishments");
                });

            modelBuilder.Entity("AuditCkDayo.Models.ExpenseSource", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("ExpenseSources");
                });

            modelBuilder.Entity("AuditCkDayo.Models.ManagerCoverage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CoveredManagerId")
                        .HasColumnType("int");

                    b.Property<int>("CoveringManagerId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("CreatedByUserId")
                        .HasColumnType("int");

                    b.Property<DateTime>("EndDate")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Reason")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Scope")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.HasIndex("CoveredManagerId");

                    b.HasIndex("CoveringManagerId");

                    b.HasIndex("CreatedByUserId");

                    b.ToTable("ManagerCoverages");
                });

            modelBuilder.Entity("AuditCkDayo.Models.Notification", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("LinkUrl")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime?>("ReadAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("Notifications");
                });

            modelBuilder.Entity("AuditCkDayo.Models.PcfRelease", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int?>("CashFlowEntryId")
                        .HasColumnType("int");

                    b.Property<int?>("EstablishmentId")
                        .HasColumnType("int");

                    b.Property<string>("Purpose")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("ReceiverName")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<int?>("ReceiverUserId")
                        .HasColumnType("int");

                    b.Property<DateTime>("ReleaseDate")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("ReleasedByTreasuryUserId")
                        .HasColumnType("int");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("CashFlowEntryId");

                    b.HasIndex("EstablishmentId");

                    b.HasIndex("ReceiverUserId");

                    b.HasIndex("ReleasedByTreasuryUserId");

                    b.ToTable("PcfReleases");
                });

            modelBuilder.Entity("AuditCkDayo.Models.PettyCashLedger", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int?>("AssociatedRecordId")
                        .HasColumnType("int");

                    b.Property<int?>("CounterpartyUserId")
                        .HasColumnType("int");

                    b.Property<string>("Notes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("ResultingBalance")
                        .HasColumnType("decimal(12,2)");

                    b.Property<DateTime>("Timestamp")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("TransactionType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CounterpartyUserId");

                    b.HasIndex("UserId");

                    b.ToTable("PettyCashLedgers");
                });

            modelBuilder.Entity("AuditCkDayo.Models.PnlCategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("Section")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("Section", "Name")
                        .IsUnique();

                    b.ToTable("PnlCategories");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SalesReport", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("BeerSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("BeverageSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<DateTime>("BusinessDate")
                        .HasColumnType("datetime(6)");

                    b.Property<decimal>("CashOut")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("CashSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("CashierName")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<int?>("CashierUserId")
                        .HasColumnType("int");

                    b.Property<decimal>("ChangeAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("ClosingGrossSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("ClosingImageUrlsJson")
                        .HasColumnType("longtext");

                    b.Property<int?>("ClosingInputtedByUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("ConfirmedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<int?>("ConfirmedByUserId")
                        .HasColumnType("int");

                    b.Property<decimal>("ConfirmedCashToHandover")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("CreditAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int>("DocumentRecordId")
                        .HasColumnType("int");

                    b.Property<decimal>("EaglesDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("EmployeeFivePercentDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("EmployeeTenPercentDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int>("EstablishmentId")
                        .HasColumnType("int");

                    b.Property<decimal>("FoodSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("GCashAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("GiftVoucherDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("GrossSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<DateTime>("HandoverDate")
                        .HasColumnType("datetime(6)");

                    b.Property<decimal>("HardSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("ImageUrlsJson")
                        .HasColumnType("longtext");

                    b.Property<decimal>("LoyaltyCardDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("Notes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("OpeningBeerSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningBeverageSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningCashSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningChangeAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningEaglesDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningEmployeeFivePercentDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningEmployeeTenPercentDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningFoodSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningGiftVoucherDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningGrossSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningHardSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int?>("OpeningInputtedByUserId")
                        .HasColumnType("int");

                    b.Property<decimal>("OpeningLoyaltyCardDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("OpeningNotes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("OpeningOtherSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningPcfFromSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningPwdDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("OpeningReceiptNumberEnd")
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("OpeningReceiptNumberStart")
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<decimal>("OpeningRestoPcf")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OpeningSalesOverageAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("OpeningSalesOverageReason")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("OpeningSalesShortageAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("OpeningSalesShortageReason")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("OpeningSeniorDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("OpeningWitnessName")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<decimal>("OtherPaymentAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("OtherSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("PcfFromSales")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("PwdDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("ReceiptNumberEnd")
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("ReceiptNumberStart")
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<decimal>("RestoPcf")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("SalesOverageAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("SalesOverageReason")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("SalesShortageAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("SalesShortageReason")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("SeniorDiscount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("WitnessName")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("CashierUserId");

                    b.HasIndex("ClosingInputtedByUserId");

                    b.HasIndex("ConfirmedByUserId");

                    b.HasIndex("DocumentRecordId");

                    b.HasIndex("EstablishmentId");

                    b.HasIndex("OpeningInputtedByUserId");

                    b.ToTable("SalesReports");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SalesReportLine", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("Label")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("LineType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<int>("SalesReportId")
                        .HasColumnType("int");

                    b.Property<int>("Section")
                        .HasColumnType("int");

                    b.Property<int>("SortOrder")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("SalesReportId");

                    b.ToTable("SalesReportLines");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SurrenderRequest", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("ActionByUserId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("ActionDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ActionNotes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<int?>("AssignedReceiverId")
                        .HasColumnType("int");

                    b.Property<int>("BuyerId")
                        .HasColumnType("int");

                    b.Property<string>("BuyerNotes")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal?>("ConfirmedAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("DeclaredAmount")
                        .HasColumnType("decimal(12,2)");

                    b.Property<DateTime>("RequestDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("ActionByUserId");

                    b.HasIndex("AssignedReceiverId");

                    b.HasIndex("BuyerId");

                    b.ToTable("SurrenderRequests");
                });

            modelBuilder.Entity("AuditCkDayo.Models.TreasuryCashFlow", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CashFlowDate")
                        .HasColumnType("datetime(6)");

                    b.Property<decimal>("ClosingBalance")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("NetCashFlow")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("StartingBalance")
                        .HasColumnType("decimal(12,2)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<decimal>("TotalCashIn")
                        .HasColumnType("decimal(12,2)");

                    b.Property<decimal>("TotalCashOut")
                        .HasColumnType("decimal(12,2)");

                    b.Property<int>("TreasuryUserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("TreasuryUserId");

                    b.ToTable("TreasuryCashFlows");
                });

            modelBuilder.Entity("AuditCkDayo.Models.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    MySqlPropertyBuilderExtensions.UseMySqlIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("DailyStartingFloat")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(12,2)")
                        .HasDefaultValue(0.00m);

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<int?>("EstablishmentId")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("tinyint(1)")
                        .HasDefaultValue(false);

                    b.Property<bool>("IsTreasury")
                        .HasColumnType("tinyint(1)");

                    b.Property<int?>("ManagerId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<decimal>("PcfBalance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(12,2)")
                        .HasDefaultValue(0.00m);

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("EstablishmentId");

                    b.HasIndex("ManagerId");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditItem", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "AssignedReviewer")
                        .WithMany()
                        .HasForeignKey("AssignedReviewerId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "Buyer")
                        .WithMany("AuditItems")
                        .HasForeignKey("BuyerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.Establishment", "Establishment")
                        .WithMany("AuditItems")
                        .HasForeignKey("EstablishmentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.User", "VerifiedBy")
                        .WithMany()
                        .HasForeignKey("VerifiedById")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("AssignedReviewer");

                    b.Navigation("Buyer");

                    b.Navigation("Establishment");

                    b.Navigation("VerifiedBy");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditItemDetail", b =>
                {
                    b.HasOne("AuditCkDayo.Models.Establishment", "AssignedEstablishment")
                        .WithMany()
                        .HasForeignKey("AssignedEstablishmentId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.AuditItem", "AuditItem")
                        .WithMany("Details")
                        .HasForeignKey("AuditItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.CostCenter", "CostCenter")
                        .WithMany()
                        .HasForeignKey("CostCenterId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.ExpenseSource", "ExpenseSource")
                        .WithMany()
                        .HasForeignKey("ExpenseSourceId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("AuditCkDayo.Models.PnlCategory", "PnlCategory")
                        .WithMany()
                        .HasForeignKey("PnlCategoryId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("AssignedEstablishment");

                    b.Navigation("AuditItem");

                    b.Navigation("CostCenter");

                    b.Navigation("ExpenseSource");

                    b.Navigation("PnlCategory");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditItemImage", b =>
                {
                    b.HasOne("AuditCkDayo.Models.AuditItem", "AuditItem")
                        .WithMany("Images")
                        .HasForeignKey("AuditItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AuditItem");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditSettlement", b =>
                {
                    b.HasOne("AuditCkDayo.Models.PcfRelease", "PcfRelease")
                        .WithMany()
                        .HasForeignKey("PcfReleaseId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "ProcessedByUser")
                        .WithMany()
                        .HasForeignKey("ProcessedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.User", "ReceiverUser")
                        .WithMany()
                        .HasForeignKey("ReceiverUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "ResponsibleManager")
                        .WithMany()
                        .HasForeignKey("ResponsibleManagerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("PcfRelease");

                    b.Navigation("ProcessedByUser");

                    b.Navigation("ReceiverUser");

                    b.Navigation("ResponsibleManager");
                });

            modelBuilder.Entity("AuditCkDayo.Models.CashBreakdownLine", b =>
                {
                    b.HasOne("AuditCkDayo.Models.SalesReport", "SalesReport")
                        .WithMany("CashBreakdownLines")
                        .HasForeignKey("SalesReportId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.Navigation("SalesReport");
                });

            modelBuilder.Entity("AuditCkDayo.Models.CashFlowEntry", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "ConfirmedByUser")
                        .WithMany()
                        .HasForeignKey("ConfirmedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.CostCenter", "CostCenter")
                        .WithMany()
                        .HasForeignKey("CostCenterId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "CreatedByUser")
                        .WithMany()
                        .HasForeignKey("CreatedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.Establishment", "Establishment")
                        .WithMany()
                        .HasForeignKey("EstablishmentId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "RelatedUser")
                        .WithMany()
                        .HasForeignKey("RelatedUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "ReportedByUser")
                        .WithMany()
                        .HasForeignKey("ReportedByUserId");

                    b.HasOne("AuditCkDayo.Models.DocumentRecord", "SourceDocument")
                        .WithMany()
                        .HasForeignKey("SourceDocumentId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.TreasuryCashFlow", "TreasuryCashFlow")
                        .WithMany("Entries")
                        .HasForeignKey("TreasuryCashFlowId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ConfirmedByUser");

                    b.Navigation("CostCenter");

                    b.Navigation("CreatedByUser");

                    b.Navigation("Establishment");

                    b.Navigation("RelatedUser");

                    b.Navigation("ReportedByUser");

                    b.Navigation("SourceDocument");

                    b.Navigation("TreasuryCashFlow");
                });

            modelBuilder.Entity("AuditCkDayo.Models.DocumentRecord", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "ConfirmedByUser")
                        .WithMany()
                        .HasForeignKey("ConfirmedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "UploadedByUser")
                        .WithMany()
                        .HasForeignKey("UploadedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ConfirmedByUser");

                    b.Navigation("UploadedByUser");
                });

            modelBuilder.Entity("AuditCkDayo.Models.ManagerCoverage", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "CoveredManager")
                        .WithMany()
                        .HasForeignKey("CoveredManagerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.User", "CoveringManager")
                        .WithMany()
                        .HasForeignKey("CoveringManagerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.User", "CreatedByUser")
                        .WithMany()
                        .HasForeignKey("CreatedByUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CoveredManager");

                    b.Navigation("CoveringManager");

                    b.Navigation("CreatedByUser");
                });

            modelBuilder.Entity("AuditCkDayo.Models.Notification", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("AuditCkDayo.Models.PcfRelease", b =>
                {
                    b.HasOne("AuditCkDayo.Models.CashFlowEntry", "CashFlowEntry")
                        .WithMany()
                        .HasForeignKey("CashFlowEntryId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.Establishment", "Establishment")
                        .WithMany()
                        .HasForeignKey("EstablishmentId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "ReceiverUser")
                        .WithMany()
                        .HasForeignKey("ReceiverUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "ReleasedByTreasuryUser")
                        .WithMany()
                        .HasForeignKey("ReleasedByTreasuryUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CashFlowEntry");

                    b.Navigation("Establishment");

                    b.Navigation("ReceiverUser");

                    b.Navigation("ReleasedByTreasuryUser");
                });

            modelBuilder.Entity("AuditCkDayo.Models.PettyCashLedger", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "CounterpartyUser")
                        .WithMany()
                        .HasForeignKey("CounterpartyUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("CounterpartyUser");

                    b.Navigation("User");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SalesReport", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "CashierUser")
                        .WithMany()
                        .HasForeignKey("CashierUserId");

                    b.HasOne("AuditCkDayo.Models.User", "ClosingInputtedByUser")
                        .WithMany()
                        .HasForeignKey("ClosingInputtedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "ConfirmedByUser")
                        .WithMany()
                        .HasForeignKey("ConfirmedByUserId");

                    b.HasOne("AuditCkDayo.Models.DocumentRecord", "DocumentRecord")
                        .WithMany()
                        .HasForeignKey("DocumentRecordId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.Establishment", "Establishment")
                        .WithMany()
                        .HasForeignKey("EstablishmentId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("AuditCkDayo.Models.User", "OpeningInputtedByUser")
                        .WithMany()
                        .HasForeignKey("OpeningInputtedByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("CashierUser");

                    b.Navigation("ClosingInputtedByUser");

                    b.Navigation("ConfirmedByUser");

                    b.Navigation("DocumentRecord");

                    b.Navigation("Establishment");

                    b.Navigation("OpeningInputtedByUser");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SalesReportLine", b =>
                {
                    b.HasOne("AuditCkDayo.Models.SalesReport", "SalesReport")
                        .WithMany("Lines")
                        .HasForeignKey("SalesReportId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SalesReport");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SurrenderRequest", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "ActionByUser")
                        .WithMany()
                        .HasForeignKey("ActionByUserId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "AssignedReceiver")
                        .WithMany()
                        .HasForeignKey("AssignedReceiverId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "Buyer")
                        .WithMany()
                        .HasForeignKey("BuyerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ActionByUser");

                    b.Navigation("AssignedReceiver");

                    b.Navigation("Buyer");
                });

            modelBuilder.Entity("AuditCkDayo.Models.TreasuryCashFlow", b =>
                {
                    b.HasOne("AuditCkDayo.Models.User", "TreasuryUser")
                        .WithMany()
                        .HasForeignKey("TreasuryUserId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("TreasuryUser");
                });

            modelBuilder.Entity("AuditCkDayo.Models.User", b =>
                {
                    b.HasOne("AuditCkDayo.Models.Establishment", "Establishment")
                        .WithMany()
                        .HasForeignKey("EstablishmentId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("AuditCkDayo.Models.User", "Manager")
                        .WithMany("StaffMembers")
                        .HasForeignKey("ManagerId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Establishment");

                    b.Navigation("Manager");
                });

            modelBuilder.Entity("AuditCkDayo.Models.AuditItem", b =>
                {
                    b.Navigation("Details");

                    b.Navigation("Images");
                });

            modelBuilder.Entity("AuditCkDayo.Models.Establishment", b =>
                {
                    b.Navigation("AuditItems");
                });

            modelBuilder.Entity("AuditCkDayo.Models.SalesReport", b =>
                {
                    b.Navigation("CashBreakdownLines");

                    b.Navigation("Lines");
                });

            modelBuilder.Entity("AuditCkDayo.Models.TreasuryCashFlow", b =>
                {
                    b.Navigation("Entries");
                });

            modelBuilder.Entity("AuditCkDayo.Models.User", b =>
                {
                    b.Navigation("AuditItems");

                    b.Navigation("StaffMembers");
                });
#pragma warning restore 612, 618
        }
    }
}
